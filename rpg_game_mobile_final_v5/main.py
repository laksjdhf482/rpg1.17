...
#top{position:absolute;left:3.5%;right:3.5%;top:2.2%;z-index:5;display:grid;grid-template-columns:minmax(0,42%) minmax(0,58%);align-items:start;gap:6px;pointer-events:none}
.info{color:#111;text-shadow:0 1px 2px rgba(255,255,255,.65),1px 0 0 rgba(255,255,255,.35),-1px 0 0 rgba(255,255,255,.35)}
#stageTitle{font-size:clamp(12px,4.0vw,24px);font-weight:900;line-height:1.05;white-space:nowrap;overflow:hidden;text-overflow:clip}
#moneyArea{text-align:right;min-width:0;white-space:nowrap;overflow:visible}
#moneyLabel{font-size:clamp(9px,2.4vw,14px);font-weight:900}
#money{font-size:clamp(18px,6.2vw,34px);font-weight:900;line-height:1.02;white-space:nowrap;letter-spacing:-.045em}
#clickIncome{font-size:clamp(8px,2.35vw,13px);font-weight:900;margin-top:3px;white-space:nowrap}
#progressArea{position:absolute;left:3.5%;right:3.5%;top:11.8%;z-index:5;pointer-events:none}
...
.float{position:absolute;z-index:15;pointer-events:none;font-size:clamp(14px,4.8vw,26px);font-weight:900;color:#111;text-shadow:1px 1px 0 rgba(255,255,255,.8),-1px -1px 0 rgba(255,255,255,.5);animation:floatUp .72s ease-out forwards;white-space:nowrap;max-width:92%;overflow:visible}
...
<div class="item"><div class="itemName">클릭 추가</div><div class="itemDesc">터치 1회당 인식 클릭 횟수 +1 · Lv.1 = 2회, Lv.2 = 3회, Lv.3 = 4회...</div><div class="itemRow"><span class="level" id="extraClickLevel">Lv.0 · ×1</span><button class="action" id="buyExtraClick" type="button"></button></div></div>
...
let money=saved?.money??0,stage=saved?.stage??0,moneyLevel=saved?.moneyLevel??0,extraClickLevel=saved?.extraClickLevel??0,cleared=saved?.cleared??false,lotteryResult=saved?.lotteryResult??"";
function fmt(n){return Math.floor(n).toLocaleString("ko-KR")+"원"}
function cost(l){return Math.floor(1000*Math.pow(1.5,l))}
function moneyBonus(){return moneyLevel<=0 ? 1000 : moneyLevel*1000 + Math.floor(moneyLevel/5)*500}
function clickCount(){return 1 + extraClickLevel}
function gainPerTouch(){return moneyBonus() * clickCount()}
function save(){localStorage.setItem("beggar_rpg_save",JSON.stringify({money,stage,moneyLevel,extraClickLevel,cleared,lotteryResult}))}
function render(){
 const s=STAGES[stage];game.style.backgroundImage=`url("${BACKGROUNDS[stage]}")`;character.src=CHARACTERS[stage];
 $("stageTitle").textContent=s.name;$("money").textContent=fmt(money);$("clickIncome").textContent="터치 +"+fmt(gainPerTouch())+(extraClickLevel>=1?" · 추가 클릭":"");
 ...
 $("extraClickLevel").textContent="Lv."+extraClickLevel+" · ×"+clickCount();$("buyExtraClick").textContent="구매 "+fmt(cost(extraClickLevel));
...
function earn(ev){
 ...
 const gain=gainPerTouch();money+=gain;const r=game.getBoundingClientRect();let x=Math.max(28,Math.min(r.width-28,ev.clientX-r.left)),y=Math.max(28,Math.min(r.height-25,ev.clientY-r.top));
 // 진행도 바 영역과 겹치지 않도록 큰 금액 표시 위치를 안전 구간으로 이동\n const progressTop=r.height*0.105, progressBottom=r.height*0.185;\n if(y>=progressTop-35 && y<=progressBottom+35) y=Math.max(progressBottom+48, r.height*0.24);\n x=Math.max(r.width*0.08,Math.min(r.width*0.92,x));
...
$("buyExtraClick").addEventListener("pointerdown",e=>{e.stopPropagation();e.preventDefault();const c=cost(extraClickLevel);if(money<c){$("notice").textContent="돈이 부족합니다";return}money-=c;extraClickLevel++;$("notice").textContent="클릭 추가 업그레이드 완료 · 추가 클릭";save();render()},{passive:false});
function lottery(){
...
function resetGame(){money=0;stage=0;moneyLevel=0;extraClickLevel=0;cleared=false;lotteryResult="";$("notice").textContent="";shop.classList.remove("open");save();render()}
